	...
	<div class="head-container">
		<div class="head-container-header">
			<div class="head-container-header-input">
				<el-input v-model="searchName" suffix-icon="search" :maxlength="12" :placeholder="placeholder" clearable
					@change="getdeptTree" />
			</div>
		...
		<!-- {{ realTreeData }} {{ state.List }} -->
		<el-tree class="mt20" :data="realTreeData" :props="props.props" :expand-on-click-node="false" ref="deptTreeRef"
			:loading="state.localLoading" node-key="id" highlight-current default-expand-all
...
const realTreeData = computed(() => {
	// @ts-ignore
	return state.List.filter(item => !noUseDictList.includes(item.description))
})
